...
export default function Products() {
  ...
  // Escapar o scroll quando aberto
  useEffect(() => {
    if (lightboxOpen) {
  ...
  }, [lightboxOpen, currentIndex, filteredProducts.length]);
    ...
    <>
      <section id="produtos" className="py-24 bg-cream relative">
        <div className="max-w-6xl mx-auto px-6 relative z-10">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`px-6 py-3 rounded-full text-sm font-bold tracking-widest uppercase transition-all duration-300 ${
                  activeTab === tab.id 
                    ? 'bg-wine text-white shadow-lg shadow-wine/30 translate-y-0' 
          ...
          {/* Banner B2B e Atacado - Agora reposicionado */}
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
